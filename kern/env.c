...
// them to switch privilege levels. 需要全局描述符表去切换特级权
//
...
void env_init(void)
{
	...
	env_free_list = NULL;
	for (int i = NENV - 1; i >= 0; i--) // 从后向前构建链表
	{
		envs[i].env_id = 0;
	...
	env_init_percpu(); // 加载全局描述符表
}

// Load GDT and segment descriptors.
// 加载全局描述符表并且初始化段寄存器gs, fs, es, ds, ss
...
env_setup_vm(struct Env *e)
{
	...
	// LAB 3: Your code here.
	p->pp_ref++;
	e->env_pgdir = (pte_t *)page2kva(p);			// 将物理页作为页目录使用
	memcpy(e->env_pgdir, kern_pgdir, PGSIZE); // 继承内核页目录
	// UVPT maps the env's own page table read-only.
	// Permissions: kernel R, user R
	e->env_pgdir[PDX(UVPT)] = PADDR(e->env_pgdir) | PTE_P | PTE_U; // 修改 UVPT 处的权限级到当前 env_pgdir
